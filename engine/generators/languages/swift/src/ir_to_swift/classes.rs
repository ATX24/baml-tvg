...
pub fn ir_class_to_swift_stream(class: &Class, lookup: &impl baml_types::baml_value::TypeLookups) -> ClassSwift {
    ClassSwift {
        name: class.elem.name.clone(),
        docstring: class
            .elem
...
fn ir_field_to_swift_stream(field: &Field, lookup: &impl baml_types::baml_value::TypeLookups) -> FieldSwift {
    let partialized = field.elem.r#type.elem.to_streaming_type(lookup);
    let swift_type = super::super::ir_to_swift::stream_type_to_swift(&partialized, lookup);
